...
from se3_transformer.model.graph import graph as build_graph
...
def make_graph_w_2nodes(xyz, pair, idx, top_k_BB=64, top_k_SC=64, kmin=32, eps=1e-6):
    ...
    G = build_graph(src, tgt, num_nodes=B*L*2).to(device)
...
def make_full_graph(xyz, pair, idx, top_k=64, kmin=9):
    ...
    src = b*L+i
    tgt = b*L+j
    G = build_graph(src, tgt, num_nodes=B*L).to(device)
